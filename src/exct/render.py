...
def CALC_SPRITE_POINTS(SPRITE_POSITION, PLAYER_POSITION, SPRITE_DIMENTIONS):
	...
	ANGLE_A = PLAYER_SPRITE_ANGLE + utils.piDIV2 #Rotate 90* (π/2 rads) ahead.
	ANGLE_B = PLAYER_SPRITE_ANGLE - utils.piDIV2 #Rotate 90* (π/2 rads) behind.
...
def SET_PYGAME_CONTEXT():
	...
	return (SCENE_SHADER, QUAD_SHADER), (VAO_QUAD, VAO_UI, FBO_SCENE, TCB_SCENE), (VAO_SCENE, VBO_SCENE, EBO_SCENE), (MODEL_MATRIX, PROJECTION_MATRIX)
...
def PROCESS_OBJECT(OBJECT_DATA, PLAYER_INSTANCE, COPIED_VERTS, COPIED_INDICES, SHEETS_USED, PHYS_DATA, CURRENT_ID, FPS):
	...
	if OBJECT_TYPE in (SPRITE_STATIC, ITEM, ENEMY, PROJECTILE, EXPLOSION,):
		#Sprites and sprite-like objects.
					...
					if type(COLLIDED_OBJECT) == PLAYER:
						...
						for _ in range(6):
							...
							if type(COLLIDED_OBJECT) in (PLAYER, ENEMY,):
								PHYS_DATA = COLLIDED_OBJECT.HURT(5.0, PHYS_DATA, CURRENT_ID)
		...
		elif OBJECT_TYPE in (EXPLOSION,):
			COORDINATES = [COORDINATE - VECTOR_3D(0.0, OBJECT_DATA.DIMENTIONS_3D.Y/4, 0.0) for COORDINATE in COORDINATES]
			TEXTURES = OBJECT_DATA.TEXTURE_INFO

		else:
			TEXTURES = OBJECT_DATA.TEXTURE_INFO

		COPIED_VERTS, COPIED_INDICES = OBJECT_VAO_MANAGER(OBJECT_DATA, [COPIED_VERTS, COPIED_INDICES], OBJECT_DATA.TEXTURE_SHEETS_USED, SHEETS_USED, TEXTURES=TEXTURES, POINTS=COORDINATES)

	elif OBJECT_TYPE in [CUBE_PHYSICS, CUBE_PATH,]:
		#Physics cubes, and other similar types.
		NORMALS = utils.FIND_CUBOID_NORMALS(OBJECT_DATA.POINTS)
		COPIED_VERTS, COPIED_INDICES = OBJECT_VAO_MANAGER(OBJECT_DATA, [COPIED_VERTS, COPIED_INDICES], OBJECT_DATA.TEXTURE_SHEETS_USED, SHEETS_USED)

	elif OBJECT_TYPE == RAY:
		#Raycasts visualised (2 Triangles)
		OBJECT_DATA.LIFETIME -= (1 / FPS)

		if OBJECT_DATA.LIFETIME > 0:
...
def OBJECT_VAO_MANAGER(OBJECT, VAO_DATA, TEXTURE_SHEETS_USED, SHEETS_USED, TEXTURES=None, POINTS=None):
	...
	try:
		...
		if VAO_VERTICES.size == 0:
			VAO_VERTICES = NP.empty((0, 9), dtype=NP.float32)
		...
		NEW_VERTICES = NP.zeros((NUM_VERTS, 9), dtype=NP.float32)
		...
		if CLASS_TYPE in (CUBE_STATIC, CUBE_PHYSICS, CUBE_PATH,): #Cube-like objects.
			...
			for FACE_INDEX, TEX_COORDS in enumerate(CUBE_TEXTURE_DATA):
				...
				TEXTURE_INDEX = SHEETS_USED.index(TEXTURE_SHEETS_USED[FACE_INDEX//2 - 1])
				...
				for I, INDEX in enumerate(FACE_INDICES):
					...
					NEW_VERTICES[FACE_INDEX * 4 + I] = [X, Y, Z, U, V, NORMAL.X, NORMAL.Y, NORMAL.Z, TEXTURE_INDEX]

				INDEX_OFFSET += 4


		elif CLASS_TYPE in (QUAD, INTERACTABLE, SPRITE_STATIC, ITEM, ENEMY, PROJECTILE, EXPLOSION,): #Quad-like objects.
			FACE_ORDER = (0, 1, 2, 3)
			...
			for I, INDEX in enumerate(FACE_ORDER):
				...
				NEW_VERTICES[I] = [X, Y, Z, U, V, NORMAL.X, NORMAL.Y, NORMAL.Z, TEXTURE_INDEX]
...
def BUFFERS_INIT(VERTICES=None, INDICES=None, NORMALS=False):
	...
	#3 position values, 2 texture coordinate values, 3 normal values, 1 sheet ID, where each value (Float/uInt) is 4 bytes
	STRIDE = (3 + 2 + 3 + 1) * 4
	OFFSET = 0
...
def RENDER_DEPTH_MAP(VAO_SHADOW, SHADOW_SHADER, PROJECTION, VIEW, FBO_SHADOW, SHADOWMAP_RESOLUTION, ENV_VAO_INDICES, SHEET_ARRAY, LIGHT):
	...
	SHEETS_ARRAY_LOC = glGetUniformLocation(SHADOW_SHADER, 'SHEETS')
...
def CREATE_SHADOW_MAPS(SURFACE, I, LIGHT, VAO_DATA, SHEETS_USED):
	...
	RENDER_DEPTH_MAP(VAO_SHADOW, SHADOW_SHADER, LIGHT_PROJECTION_MATRIX, LIGHT_VIEW_MATRIX, FBO_SHADOW, SHADOWMAP_RESOLUTION, VAO_DATA[1], SHEET_ARRAY, LIGHT)
